...
/*   Updated: 2025/06/10 15:59:43 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d_bonus.h"

static inline void	print_hp_bar(t_img *hp_bar, int *pixels_hp_bar, int hp_bar_h, int hp_bar_w)
{
	int i;
	int	width;
	int	height;
...
static inline void	put_pixels_hp_bar(int hps_left, t_stats *p_stats, int width, int height)
{
	int i;
...
void	display_stats(t_stats p_stats, t_mlx mlx)
{
	...
	print_hp_bar(p_stats.hp_bar, p_stats.pixels_hp_bar, p_stats.hp_bar_h, p_stats.hp_bar_w);
...
void	init_hp_bar(t_stats *p_stats, t_mlx mlx)
{
	...
	p_stats->hp_bar = mlx_new_image(mlx.init, p_stats->hp_bar_w,
			p_stats->hp_bar_h);
	p_stats->pixels_hp_bar = malloc(sizeof(int) * (p_stats->hp_bar_h * p_stats->hp_bar_w));
